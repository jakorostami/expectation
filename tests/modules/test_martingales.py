...
class TestMixtureMartingales:
    def test_normal_mixture(self):
        ...
        one_sided_values = [one_sided.log_superMG(s, v) for s, v in zip(s_values, v_values)]
        two_sided_values = [two_sided.log_superMG(s, v) for s, v in zip(s_values, v_values)]
    ...
    def test_beta_binomial_mixture(self):
        mixture = BetaBinomialMixture(v_opt=1.0, alpha_opt=0.05, g=0.5, h=0.5, is_one_sided=True)

        # Test with various inputs
        s_values = np.linspace(-1, 1, 100)
        v_values = np.ones_like(s_values)

        log_values = [mixture.log_superMG(s, v) for s, v in zip(s_values, v_values)]
    ...
    def test_gamma_exponential_mixture(self):
        mixture = GammaExponentialMixture(v_opt=1.0, alpha_opt=0.05, c=1.0)
...
@pytest.mark.parametrize("v_opt,alpha_opt", [(0.5, 0.05), (1.0, 0.01), (2.0, 0.1)])
