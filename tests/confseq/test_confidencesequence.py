...
def basic_config():
    return ConfidenceSequenceConfig(
        alpha=0.05, alpha_opt=0.05, v_opt=1.0, c=1.0, boundary_type=BoundaryType.NORMAL_MIXTURE
    )
...
def confidence_sequence(basic_config):
    return ConfidenceSequence(config=basic_config, estimand=EstimandType.MEAN)


@pytest.fixture
def bernstein_sequence(bernstein_config):
    return EmpiricalBernsteinConfidenceSequence(config=bernstein_config, estimand=EstimandType.MEAN)
...
class TestConfidenceSequence:
    ...
    def test_initialization(self, basic_config):
        cs = ConfidenceSequence(config=basic_config, estimand=EstimandType.MEAN)
...
class TestEmpiricalBernsteinConfidenceSequence:
    def test_initialization(self, bernstein_config):
        cs = EmpiricalBernsteinConfidenceSequence(
            config=bernstein_config, estimand=EstimandType.MEAN
        )
