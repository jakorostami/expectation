...
- **Sparse-mixture merging** (`modules/merging.py`, `SparseMixtureMerger`) — the anytime-valid sparse-anomaly test of Pérez-Ortiz, Castro & Stoepker (2025) as a martingale merging function over per-stream e-processes; powerful where the arithmetic-mean merge is powerless. Requires independent streams
...
Worked examples: `examples/imprecise-forecasters-upper-expectations.ipynb`, `examples/sparse-anomaly-global-null.ipynb`, `examples/stein-score-only-goodness-of-fit.ipynb`.
