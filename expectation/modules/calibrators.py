...
class PToECalibrator:
    ...
    def __init__(
        self, calibrator_type: PToECalibratorType = PToECalibratorType.SHAFER, kappa: float = 0.5
    ):
        """
    ...
    def _power(self, p: Union[float, NDArray]) -> Union[float, NDArray]:
        ...
        """
        p = np.asarray(p)
        result = np.where(p <= 1, self.kappa * np.power(np.maximum(p, 1e-300), self.kappa - 1), 0.0)
    ...
    def _mixture(self, p: Union[float, NDArray]) -> Union[float, NDArray]:
        ...
        )  # TODO: this can be unstable for p close to 1, need to fix
        result = np.where(p == 0, np.inf, result)
        return result if result.shape else float(result)
