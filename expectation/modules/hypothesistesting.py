...
from expectation.modules.epower import EPowerCalculator, EPowerConfig, EPowerResult, EPowerType
...
class EProcessConfig(EValueConfig):
    ...
    gamma: float = Field(
        default=0.5, gt=0, le=1, description="For adaptive/conservative strategies"
    )
    conservative_lambda: float = Field(
        default=0.5, gt=0, le=1, description="Fixed lambda for conservative"
    )
...
class EValue(ABC):
    ...
    def __init__(self, null_hypothesis: Hypothesis, config: Optional[EValueConfig] = None):
...
class LikelihoodRatioEValue(EValue):
    class Config(BaseModel):
        log_space: bool = Field(
            default=True, description="Compute in log space for numerical stability"
        )
        model_config = ConfigDict(frozen=True)
...
class SymmetryETest:
    ...
    def compute_e_power(
        self, alternative_data: NDArray, e_power_config: Optional[EPowerConfig] = None
    ) -> EPowerResult:
