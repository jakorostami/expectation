...
class EProcessUpdater:
    ...
    def _create_combiner(
        self, config: Union[EValueConfig, EProcessConfig]
    ) -> SequentialEValueCombiner:
    ...
    def set_log_optimal_expectation(
        self, expectation_func: Callable[[float, List[float], int], float]
    ):
        """
        ...
        """
        if (
            isinstance(self.config, EProcessConfig)
            and self.config.betting_strategy == BettingStrategy.LOG_OPTIMAL
        ):
    ...
    def compute_asymptotic_growth_rate(
        self, process: EProcess, min_samples: int = 20
    ) -> Optional[float]:
        """
