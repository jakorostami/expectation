...
def normal_log_mixture(
    s: ArrayLike, v: ArrayLike, v_opt: float, alpha_opt: float = 0.05, is_one_sided: bool = True
) -> np.ndarray:
    s, v = np.asarray(s), np.asarray(v)
    mixture = (
        OneSidedNormalMixture(v_opt, alpha_opt)
        if is_one_sided
...
def normal_mixture_bound(
    v: ArrayLike, alpha: float, v_opt: float, alpha_opt: float = 0.05, is_one_sided: bool = True
) -> np.ndarray:
    v = np.asarray(v)
    mixture = (
        OneSidedNormalMixture(v_opt, alpha_opt)
...
def gamma_exponential_log_mixture(
    s: ArrayLike, v: ArrayLike, v_opt: float, c: float, alpha_opt: float = 0.05
) -> np.ndarray:
    s, v = np.asarray(s), np.asarray(v)
...
def gamma_exponential_mixture_bound(
    v: ArrayLike, alpha: float, v_opt: float, c: float, alpha_opt: float = 0.05
) -> np.ndarray:
    v = np.asarray(v)
    mixture = GammaExponentialMixture(v_opt, alpha_opt, c)
    return np.vectorize(lambda x: mixture.bound(x, np.log(1 / alpha)))(v)
...
def gamma_poisson_mixture_bound(
    v: ArrayLike, alpha: float, v_opt: float, c: float, alpha_opt: float = 0.05
) -> np.ndarray:
    v = np.asarray(v)
    mixture = GammaPoissonMixture(v_opt, alpha_opt, c)
...
def beta_binomial_mixture_bound(
    v: ArrayLike,
    alpha: float,
    v_opt: float,
    g: float,
...
def poly_stitching_bound(
    v: ArrayLike, alpha: float, v_min: float, c: float = 0, s: float = 1.4, eta: float = 2
) -> np.ndarray:
    v = np.asarray(v)
...
def empirical_process_lil_bound(
    t: Union[int, float], alpha: float, t_min: float, A: float = 0.85
) -> float:
    bound = EmpiricalProcessLILBound(alpha, t_min, A)
    return bound(t)


def double_stitching_bound(
    quantile_p: float,
    t: float,
    alpha: float,
    t_opt: float,
    ...
    r = (
        quantile_p
        if quantile_p >= 0.5
        else min(0.5, expit(logit(quantile_p) + 2 * delta * np.sqrt(t_opt * eta / t_max_m)))
    )
    ...
    ell = (
        s * np.log(np.log(eta * t_max_m / t_opt))
        + s * np.log(j)
    ...
    return (
        delta * np.sqrt(eta * t_max_m * sigma_sq / t_opt)
        + np.sqrt(k1 * k1 * sigma_sq * t_max_m * ell + term2 * term2)
        + term2
    )


def bernoulli_confidence_interval(
    num_successes: Union[float, int],
    ...
    lower_bound = 0.0
    if empirical_p > 0:
        lower_bound = optimize.bisect(lambda p: objective(p), 0.0, empirical_p, xtol=2**-40)

    # Find upper bound
    upper_bound = 1.0
    if empirical_p < 1:
        upper_bound = optimize.bisect(lambda p: -objective(p), empirical_p, 1.0, xtol=2**-40)
