...
warnings.warn(
    "The conformal module is experimental and may change in future releases.\nIt comes with no validation and is not tested by the core team.",
    UserWarning,
)
    ...
    def __init__(
        ...
        if use_conformal_calibration:
            ...
            if nonconformity_type == "normal":
                self.mixture = (OneSidedNormalMixture if is_one_sided else TwoSidedNormalMixture)(
                    v_opt, alpha_opt
                )
    ...
    def _update_with_mixture(self, data: np.ndarray) -> float:
        ...
        for x in data:
            ...
            if self._n_samples > 1:
                ...
                self._running_var = ((self._n_samples - 2) * self._running_var + delta * delta2) / (
                    self._n_samples - 1
                )
...
class ConformalEPseudomartingale:
    ...
    def __init__(self, initial_capital: float = 1.0, allow_infinite: bool = False):
    ...
    def get_history(self) -> Tuple[np.ndarray, np.ndarray]:
        return (np.array(self._e_values), np.array(self._capital_history))
...
class TruncatedEPseudomartingale(ConformalEPseudomartingale):
    def __init__(
        self, initial_capital: float = 1.0, min_capital: float = 1e-10, allow_infinite: bool = False
    ):
