...
class AdaptiveThresholdHandler:
    ...
    def __init__(
        ...
        self.min_threshold = (
            min_threshold  # Ensure that the threshold doesn't drop below this level
        )
    ...
    def update(self, current_rate: float) -> float:
        ...
        if self.cumulative_error > 0:
            adjustment_factor = 1 + (
                self.learning_rate * abs(self.cumulative_error)
            )  # Increase more conservatively if below target
        else:
            adjustment_factor = 1 / (
                1 + (self.learning_rate * abs(self.cumulative_error))
            )  # Decrease more aggressively if above target
