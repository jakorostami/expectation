...
class TtestGaussianMixtureMartingale(MixtureSupermartingale):
    ...
    def log_superMG(self, s: float, v: float) -> float:
        ...
        log_second_term = (n / 2) * np.log(
            (n + self.c_squared) * v / ((n + self.c_squared) * v - s**2)
        )  # -inf problem
            ...
            if ttest_method == "universal_inference":
                ...
                def calculator(data):
                    ...
                    if test.alternative == AlternativeType.TWO_SIDED:
                        ...
                        for i in range(n):
                            standardized_terms[i] = (
                                -0.5
                                * (
                                    (data_centered[i] - test._mu_estimators[-n - 1 + i])
                                    / test._sigma_estimators[-n - 1 + i]
                                )
                                ** 2
                            )
                        ...
                        if test.alternative == AlternativeType.LESS:
                            return np.exp(
                                log_process.conjugate()
                                if np.iscomplexobj(log_process)
                                else log_process
                            )
                        return np.exp(log_process)
    ...
    def update(self, new_data: np.ndarray) -> Tuple[float, float]:
        ...
        if self.method == "universal_inference":
            ...
            for i in range(min(n, len(self.mu_estimators) - 1)):
                log_sum_terms += (
                    np.log(self.sigma_estimators[i] ** 2)
                    + ((new_data[i] - self.mu_estimators[i]) / self.sigma_estimators[i]) ** 2
                )
...
def create_ttest(
    ...
    test = SequentialTesting(
        test_type=TestType.TTEST, null_value=null_value, alternative=alternative, config=config
    )

    return setup_ttest(test)


def create_ttest_confidence_sequence(
    alpha: float = 0.05, method: str = "universal_inference", prior_precision: float = 1.0
